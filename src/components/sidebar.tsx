...
export default function Sidebar({ mobileOpen, onMobileClose }: Props) {
  ...
  const content = (
    <>
      <div>
        ...
        <button
          onClick={() => { setShowNewProject(true); onMobileClose?.(); }}
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <nav className="md:hidden bg-surface-container-lowest h-screen w-64 fixed left-0 top-0 flex flex-col p-6 gap-8 z-30 overflow-y-auto animate-[slideInLeft_0.2s_ease]">
          {content}
        </nav>
      )}
    </>
